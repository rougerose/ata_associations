...
function genie_ata_importer_traiter_sources($t) {
  // Adapté de mailshot/genie/mailshot_bulksend
  spip_log("ata_importer_traiter_sources:meta_processing:".$GLOBALS['meta']['ata_import_processing'],"ata_traiter_sources." . _LOG_INFO_IMPORTANTE);
    ...
    if (!$nb = intval($nb)) {
      ...
      if (!$boost) {
        ...
        if ($last = intval($last) and ($dt = $now - $last) > 0) {
          ...
          spip_log("Correction sur nb : $c ($dt au lieu de $periode) => $nb","ata_traiter_sources." . _LOG_INFO_IMPORTANTE);
        }
      }
      ecrire_fichier($f_last, $now);
    }

    include_spip('inc/ata_importer_associations_sources');
    $restant = ata_importer_associations_traiter_sources($nb);
